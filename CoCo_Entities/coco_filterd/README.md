...
- The annotations file are keeping the same format as the original CoCo annotations, here is an example from the `coco_karpathy_test.json`:
...
- The entities file are aligned with the annotations file, where each line is a json object, here is an example from the `coco_entities_test.json`:
